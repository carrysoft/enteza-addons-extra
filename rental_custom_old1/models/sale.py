...
class DeliveryOrder(models.Model):
    ...
    def create_sale_order(self):
        # Crea una nueva instancia del modelo de la orden de venta
        sale_order = self.env['sale.order'].create({
            'partner_id': self.partner_id.id, # ID del cliente de la orden de entrega
            'order_line': [(0, 0, {
            ...
            }) for line in self.move_lines if line.product_id.type == 'product'], # Líneas de pedido de la orden de venta
            'picking_ids': [(6, 0, [self.id])], # IDs de las órdenes de entrega relacionadas
            'origin': self.name, # Origen de la orden de venta
            'date_order': fields.Datetime.now(), # Fecha de la orden de venta
            # Agrega otros campos relevantes de la orden de venta aquí
        })
        # Abre el formulario de la nueva orden de venta
        return {
...
class SaleOrder(models.Model):
    ...
    delivery_ids = fields.Many2many('stock.picking', 'sale_order_delivery_rel', 'order_id', 'delivery_id', string="Órdenes de entrega")
